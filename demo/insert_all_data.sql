...
-- 12. 初始化预测模型数据
INSERT INTO prediction_models (model_name, model_type, description, status) VALUES 
('ARIMA模型', 'ARIMA', '自回归积分滑动平均模型，适用于时间序列预测', 'ACTIVE'),
('Prophet模型', 'PROPHET', 'Facebook开发的预测工具，适用于具有明显季节性特征的数据', 'ACTIVE'),
('XGBoost模型', 'XGBOOST', '梯度提升决策树模型，适用于结构化数据预测', 'ACTIVE'),
('LSTM模型', 'LSTM', '长短期记忆网络，适用于复杂时间序列预测', 'ACTIVE');
