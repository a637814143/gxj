...
SELECT r.id, p.id FROM roles r, permissions p WHERE r.name = 'FARMER' AND p.name = 'crop_data:read';

INSERT INTO role_permissions (role_id, permission_id) 
SELECT r.id, p.id FROM roles r, permissions p WHERE r.name = 'FARMER' AND p.name = 'prediction:read';

INSERT INTO role_permissions (role_id, permission_id) 
SELECT r.id, p.id FROM roles r, permissions p WHERE r.name = 'RESEARCHER' AND p.name IN ('crop_data:read', 'crop_data:create', 'prediction:read', 'prediction:create');
...
-- 插入测试预测结果
INSERT INTO prediction_results (model_id, crop_type, region, year, predicted_yield, accuracy_score) VALUES 
(1, '小麦', '北京市', 2024, 550.75, 0.85),
(2, '水稻', '黑龙江省', 2024, 1850.25, 0.92);
